	...
	function renderCalendar(startMonth, stopMonth, year) {
		...
		$("#caltitle").text(year);
		$("title").text(year);
		$('#report').attr('href', window.location.origin + '/events/download/report/' + (year));
		$('#request').attr('href', window.location.origin + '/events/download/request/' + (year));
	...
	$(window).on('hashchange', function() {
		...
		query(window.location.origin + '/events' , {year})
			.then(data => {
				...
				$('.go').click(function() {
					window.location.href = window.location.origin + '/events/edit/' + $(this).data('id');
				});
				$('.empty').click(function() {
					...
					window.location.href = window.location.origin + '/events/add?date=' + date;
				});
				$(".go").hover(function() {
					let id = $(this).data('id');
					if ($(this).find('.tooltip').length === 0)
					query(window.location.origin + '/events/view' , {id})
					.then(data => {
						$(this).append(
							"<div class='tooltip'>" +
